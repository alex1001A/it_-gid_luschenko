...
function t07() {
  ...
  const opt71 = document.createElement("option");
  opt71.value = 22;
  opt71.textContent = "Лето II Атрейдес";

  const opt72 = document.createElement("option");
  opt72.value = 33;
  opt72.textContent = "Дункан Айдахо";

  s7.append(opt71, opt72);
  out7.append(s7);
}
...
function t09() {
  ...
  let text = document.createTextNode("Dune is a 1965 epic science fiction novel by American author Frank Herbert, originally published as two separate serials in Analog magazine.");
  details.append(text);
